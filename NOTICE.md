...
- `NOTICE.md`: this file.
- `.github/workflows/`: upstream Gitea CI and cron workflows removed, because they need Gitea's runners and secrets. Added `thally-track.yml`, generated by `thally track setup` (path filters changed from `dir` to `dir/**` so GitHub matches files inside), which dispatches merged pull requests to the `gitlite-docs` Thally docs agent.
